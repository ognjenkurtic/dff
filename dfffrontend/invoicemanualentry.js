...
// Getting api_key from localStorage and setting value of input
const getApiKey = () => {
  const api_key_ls = localStorage.getItem("api_key");
  document.getElementById("api_key").value = api_key_ls || "";
};
window.onload = getApiKey;

// Setting entered api_key input value to localStorage
inputApiKey.addEventListener("change", (event) => {
...
btnChkDups.addEventListener("click", async function (event) {
  ...
  const response = await fetch(`${apiUrl}/api/Signatures/check`, {
    ...
    method: "POST",
    body: JSON.stringify(
      prepareReqBodyFromSignatureSets([generatedSignatureSet])
    ),
  });
...
btnSendSig.addEventListener("click", async function (event) {
  ...
  setTimeout(async () => {
    // Given 0.5 sec for UI to update before confirm
    if (hasDups) {
      const isConfirmed = confirm(
        "Pronađeni su duplikati. Da li ste sigurni da želite da nastavite?"
      );
    ...
    const responseStore = await fetch(
      `${apiUrl}/api/Signatures/checkandstore`,
      {
        headers: fetchApiKeyAndPrepareHeaders(),
        method: "POST",
        body: JSON.stringify(
...
async function performSignatureGenerationAndUpdateUI() {
  ...
  const generatedSignatureSet = await generateSignatures(
    matBrojDob,
    matBrojKupac,
    brojFakture,
    datumIzdavanja,
...
async function processErrorResponse(response) {
  ...
  switch (response.status) {
    ...
    case 400:
      result.textContent =
        "Nevalidan zahtev. Proverite konzolu za više detalja.";
      break;
    default:
      result.textContent =
        "Došlo je do greške. Proverite konzolu za više detalja.";
      break;
  }
...
async function processOkResponse(response, isStoreAction) {
  ...
  if (signatureSetCheckResult.hasDuplicates) {
    ...
    const sig1Duplicate =
      signatureSetCheckResult.signatureDuplicateResponses.find(
        (sdr) => sdr.signatureType === 1
      );
    const sig2Duplicate =
      signatureSetCheckResult.signatureDuplicateResponses.find(
        (sdr) => sdr.signatureType === 2
      );
    const sig3Duplicate =
      signatureSetCheckResult.signatureDuplicateResponses.find(
        (sdr) => sdr.signatureType === 3
      );
    const sig4Duplicate =
      signatureSetCheckResult.signatureDuplicateResponses.find(
        (sdr) => sdr.signatureType === 4
      );
    const sig5Duplicate =
      signatureSetCheckResult.signatureDuplicateResponses.find(
        (sdr) => sdr.signatureType === 5
      );
